...
@objc class SQLColumn : ReadableRepositoryCol {
...
@objc class SQLRow : ReadableRepositoryRow {
	var data = Dictionary<String, ReadableRepositoryCol>()
...
@objc class SQLiteDB : ReadableRepository {
	let DB_NAME = "london.sql"
    ...
    class func sharedInstance( filename: String ) -> SQLiteDB! {
		dispatch_once(&Static.token) {
            Static.instance = self(filename: filename, gid:"")
		}
		return Static.instance!
	}
	
    class func sharedInstance(filename: String, gid:String) -> SQLiteDB! {
		dispatch_once(&Static.token) {
            Static.instance = self(filename: filename, gid:gid)
		}
		return Static.instance!
	}
    ...
    required init(filename: String, gid:String) {
		...
		var docDir = ""
        let path = NSBundle.mainBundle().pathForResource(filename, ofType: "sqlite")!		// Is this for an app group?
		if GROUP.isEmpty {
	...
	func query(sql:String, parameters:[AnyObject]?=nil)->[ReadableRepositoryRow] {
		var rows = [SQLRow]()
		dispatch_sync(queue) {
			let stmt = self.prepare(sql, params:parameters)
			if stmt != nil {
